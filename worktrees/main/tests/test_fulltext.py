...
class TestMCPFulltextEndpoint:
    ...
    @pytest.fixture
    def setup_local_router(self):
        """Set up client_router to use local client for tests."""
        import src.mcp_server

        # Create router with local settings
        local_settings = Settings(
            zotero_local=True,
            zotero_library_id="0",
            zotero_library_type="user",
        )
        test_router = ZoteroClientRouter(settings=local_settings)
        ...
        src.mcp_server.client_router = original_router
