...
class TestTextChunkerBoundary:
    ...
    def test_just_over_boundary(self) -> None:
        """Test text that's just over the 20000 token boundary."""
        chunker = TextChunker(max_tokens=20000)

        # Create text that's 20001 tokens (80004 chars)
        over_text = "a" * 80004
    ...
    def test_very_long_single_sentence(self) -> None:
        ...
        # Should still chunk it somehow (even if not at sentence boundaries)
        if chunker.estimate_tokens(long_sentence) > 1000:
            assert result["has_more"] is True
    ...
    def test_paragraph_boundary_preservation(self) -> None:
        ...
        # Check if last characters are sentence ending or paragraph break
