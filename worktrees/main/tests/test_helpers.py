...
class ZoteroTestDataManager:
    ...
    async def batch_create_collections(
        ...
        """
        from src.models import CollectionCreate
    ...
    async def verify_item_count(self, collection_key: str, expected_count: int) -> bool:
        ...
        """
        items = await self.client.get_collection_items_list(collection_key)
    ...
    def cleanup_entire_library(self) -> dict[str, int]:
        ...
        # Get all item keys using new API with pagination support
        # Items from iterator are raw dicts with key + version
        for item in self.client.items:
            try:
                self.client._client.delete_item(item)
                deleted["items"] += 1
            except Exception:
                pass

        # Get all collections using new API with pagination
