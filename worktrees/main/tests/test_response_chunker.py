...
from src.models import ZoteroItem, ZoteroItemData, ZoteroTag
...
class TestResponseChunker:
    ...
    def test_chunker_constants(self) -> None:
        ...
        assert chunker.MCP_TOKEN_LIMIT == 25000
    ...
    def test_needs_chunking_with_safety_margin(self) -> None:
        ...
        # Create items that total ~19000 tokens (over effective_max but under max_tokens)
        # Each item is ~209 tokens, so 91 items = ~19000 tokens
        items = [self.create_test_item(f"TEST{i}") for i in range(91)]

        # Should need chunking because effective_max_tokens = 18000
        assert chunker.needs_chunking(items) is True

        # Create items that total ~17000 tokens (81 items = ~16929 tokens)
        small_items = [self.create_test_item(f"TEST{i}") for i in range(81)]
    ...
    def test_get_next_chunk_invalid_id(self) -> None:
        ...
        response = chunker.get_next_chunk("invalid-chunk-id")

        assert response.error == "Invalid or expired chunk ID"
        assert response.has_more is False
